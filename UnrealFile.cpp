...
meta = (BindWidget) - UUserWidget을 상속한 클래스에서 팔레트에 넣을 변수들을 선언할 때 반드시 UPROPERTY()안에 들어가야하는 매크로값.
                      WBP안에서 해당 클래스와 같은 타입의 변수를 만들고 이름의 대소문자까지 맞추지 않으면 에러 발생.
meta = (BindWidgetOptional) - BindWidget과는 달리 WBP와 연동되지 않아도 에러가 뜨지 않는다. 이점이 장점같으나 실제로는 단점이다.
...
// 위젯 컴포넌트에 연동된 위젯 클래스에 대한 포인터를 받을 수 있다. 위젯 컴포넌트는 거의 액터에 접근해야하는데 다양한
// 방법중 그냥 UUserWidget을 상속한 사용자 정의 클래스를 만들어 그 안에 프로그레스 바 같은 변수들을 직접 선언하여
// WBP에서 액터로의 접근이 아닌 액터에서 WBP에 대해 이벤트 접근과도 같이 구현하였다. 이 때 반드시 필요하며 왠만해선
// 캐스팅이 필요하다.
...
// 아주 중요한 특성중 하나가 바로 로직만 들고 있되, 자료는 들어서는 안된다는 것이다. 상술하였듯 하나의 객체를 공유하기 때문에 자료를 가지고 있어봤자
...
// 기존에 GameInstance에 아이템을 담당하는 ItemDate를 캐싱하고 있었는데 굳이 이럴 필요없이 새로 UGameInstanceSubsystem을
// 상속하여 아이템을 담당하는 클래스를 만드는 것이 코드와 역할을 더 자세히 분리할 수 있는 방법이다. 나눌 수 있는건 전부 나누자.

// UGameInstanceSubsystem와 상속한 클래스에서는 어차피 GameInstance의 아래에 있기에 GetWorld() 없이 GetGameInstance()를
// 호출할 수 있다.

// 어떤 포인터 변수를 Destroy()로 파괴한다면 반드시 nullptr처리까지 해주어야한다. 삭제되는 건 포인터안에 변수이며 포인터값 자체는
// 전혀 변함이 없기에 if()로 전혀 걸러낼 수 없기 때문이다.

// ACharacter 클래스처럼 스태틱 메시가 기본적으로 있는 경우가 아닌 폰이나 액터의 경우 스태틱 혹은 스켈레톤 메시를 넣은 후에 반드시
// Mesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);를 해야 한다. 그렇지 않으면 메시와 충돌 연산이 일어난다.

// 소켓의 위치를 조절할 때에는 스켈레톤 메시 에디터에서 소켓을 우클릭하여 프리뷰 에셋을 추가하면 굉장히 편하다. 
// 또한 애니메이션에까지 연동되어 더욱더 사용하기 편하다.
...
// meta = AllowPrivateAccess가 영향을 미치는건 오직 BP의 경우다. 에디터에서는 언제든지 private 변수에 대해 접근이 가능하며
// 이게 UPROPERTY() 리플렉션의 힘이다.
...
// 앞에 class만 간단하게 넣어주면 알아서 되었다. 반환값, 선언된 변수, 함수의 매개변수 무엇이든 상관없으나 반드시 참조자나 포인터인
// 경우에만 가능하다는 것을 잊지말자. 단순히 비포인터라면 그 크기를 알아야하기 때문에 반드시 헤더를 인클루드 해야한다.

// In Place가 없는 애니메이션이 루트 모션을 켜도 애니메이션 재생중 위치가 바뀔때에는 루트 모션 아래에 
// 루트 강제 잠금(Force Lock Root)를 키니 괜찮아졌다.

// ABP의 각 스테이트간의 이동인 트랜지션에 사용되는 조건을 위해 변수를 사용하지 않는 방법을 꾀했으나 실패했는데
// TryGetPawnOwner()같은 함수는 메인 스레드에서만 호출이 가능하기 때문이다. 결국 변수를 사용해야하는데
// 기존처럼 c++파일로 AninInstance를 상속받아 변수를 선언할 수도, ABP에서 선언할 수도 있다.
// 전자는 대규모에서 처리가 편하고 후자는 간략하게 할 수 있다.

// 애니메이션이 제대로 재생되지 않을 때가 있다. Idle이나 Run같은 경우인데 이럴 땐 원본 애니메이션 시퀀스의 루프는
// 전혀 상관이 없고 스테이트 머신안에 있는 스테이트 안에 있는 시퀀스의 루프를 설정해야만 한다.

// 애니메이션 몽타주를 사용할 때 기존에는 하나의 몽타주안에 관련된 모든 애니메이션을 다 집어넣고 재생 이후 점프하는 방식을 사용하였는데
...
// 위젯의 가시화는 그 타이밍에 유의해야한다. 예를 들어 이번의 경우 자식은 히든으로 시작하고 부모는 건드리지 않았을 때 부모의 visible ->
// 자식의 visible이 꼬여서 후자가 반영이 되지 않았다. 아마 이럴 경우는 거의 없을 거라 생각하지만 텍스트의 경우 이러하였다. 텍스트가
// 자식이었다. 이런 경우 무조건 텍스트의 초기화 값을 " "으로 하면 아무것도 보이지 않기에 무적이다. 2026 6월 14일을 읽어보라.

// 데이터테이블을 구현할 때에는 에디터에서 클래스를 만드는 게 아닌 VS에서 직접 만들어야한다. 이럴 때 경로가 Intermediate안으로 잡히는
// 버그같은 경우가 존재한다. 만약 구조체 파일을 만들고 인클루드했는데 경로를 못찾겠다라는 에러가 뜰 경우 확인해보자.